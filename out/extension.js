...
async function activate(context) {
    ...
    const processtest = vscode.commands.registerCommand('karatefeatureautogen.processTest', () => {
        ...
        let currentPanel = vscode.window.createWebviewPanel('processTests', 'Karate Feature Auto Generator', vscode.ViewColumn.One, { enableScripts: true, });
...
function generateKarateFeature(swaggerSpec, outputPath) {
    let parsedSpec;
    ...
    try {
        if (swaggerSpec.trim().startsWith('{')) {
            ...
            parsedSpec = js_yaml_1.default.load(swaggerSpec); // YAML spec
        }
...
function generateMockData(schema) {
    ...
    switch (schema.type) {
        ...
        case 'object':
            return Object.fromEntries(Object.entries(schema.properties || {}).map(([key, subschema]) => [key, generateMockData(subschema)]));
        default:
            return null;
    }
}
